...
\subsubsection{CT-Exh structure is compatible with SCs}
English SCs are most naturally uttered with intonational stress on some part of their subject as shown in \Next.
\ex.
\a. A building on campus no-one \textsc{likes} is Robarts.
\b. A building on campus \textsc{no-one} likes is Robarts.
\b. A building on \textsc{campus} no-one likes is Robarts.
\b. A building \textsc{on} campus no-one likes is Robarts.
\b. A \textsc{building} on campus no-one likes is Robarts.
...
If DP2 is Foc-Marked, and the stressed constituent \textit{no one} is CT-Marked, then we can use \citeauthor{buring2003d}'s (\citeyear{buring2003d}) CT-value formation procedure to construct the d-tree associated with it.
...
If we wish to retain \citeauthor{mikkelsen2004specifying}'s (\citeyear{mikkelsen2004specifying}) analysis, the natural move would be to claim that licit SC subjects are characterized by aboutness.
...
One might consider the possibility that it is the pronominal subject of \Last[a] that forces its identificational reading.
...
Therefore, CT marking on the nominal is equivalent to CT marking on the entire DP.
