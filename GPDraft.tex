...
I propose that SCs necessarily have a CT+F structure, with the postcopular DP as focus and some element of the subject as CT.
...
\Textcite{swart2007bare} argues that the capacities are individuals (type $e$) that are sortally distinct from kinds \parencite{carlson1977reference} and the two sorts are coverted to predicates (type $\langle e,t\rangle$) by distinct operators (\textsc{Cap} and \textsc{Rel}, respectively).
I propose that, while \textsc{Rel}$(x)$ introduces a contextual variable, \textsc{Cap}$(x)$ does not.
This distinction is important if we assume, following \textcite{fintel2004minimal}, that quantifier restrictors are anaphoric
...
Indefinite SCs do not carry any implication that the individual referred to by the object DP exhausts the preoperty defined by the subject, as can be seen in \ref{ex:ObjNotExh} below.
\ex.\label{ex:ObjNotExh}

There is, however an exhaustivity implication encoded in Indefinte SCs.
The subject of SCs encodes a property that exhausts the relevant properties of the Object DP.
That is an SC of the form $[a [\alpha [\beta]_{CT} \gamma]]$\textit{ is }\textsc{name}, implies that there is no property $\beta^\prime$ such that $\beta^\prime$ holds of \textsc{name} in the context of $\alpha$ and $\gamma$.

\ex.\label{ex:SubjExh} \textbf{Context: }The adressee has just been injured in a car accident. Dave happens to be a doctor and a lawyer. Graham is also a lawyer.
	\a. A good [doctor]$_{CT}$ to talk to is Dave.
		\a.\# A good [lawyer]$_{CT}$ to talk to is Dave (too).
		\b. A good [lawyer]$_{CT}$ to talk to is Graham
		\z.
	\z.
